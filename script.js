...
function renderBasketDishes() {
  ...
  for (
    let indexBasketDishes = 0;
    indexBasketDishes < basketDishes.length;
    indexBasketDishes++
  ) {
...
function addToBasket(arrayName, index) {
  ...
  if (existingDish) {
    ...
    basketDishes.push(
      basketInputTemplate(dish.title, dish.price, dish.quantity)
    );
  }
  init();
}
...
function sumOfDish(indexBasketDishes) {
  return (
    basketDishes[indexBasketDishes].price.toFixed(2) *
    basketDishes[indexBasketDishes].quantity
  );
}
...
function deleteDish(indexBasketDishes) {
  let basketDishesRef = document.getElementById(
    `basket-dishes${indexBasketDishes}`
  );
  basketDishesRef.innerHTML = "";
  basketDishes.splice(indexBasketDishes, 1);
  renderBasketDishes();
...
function toggleRespBasket() {
  document
    ...
    .classList.replace("basket-side-layout", "rep-basket-overlay");
}

function toggleRespBasketOff() {
  document
    .getElementById("basket-overlay")
    .classList.replace("rep-basket-overlay", "basket-side-layout");
}
